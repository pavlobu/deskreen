...
export default class VideoAutoQualityOptimizer {
  ...
  findAndSetVideoInternalVariable(document: Document) {
    this.video = document.querySelector(`#${PLAYER_WRAPPER_ID} > video`) as HTMLVideoElement;
  }

  findAndSetCanvasInternalVariable(document: Document) {
    this.canvas = document.querySelector(`#${COMPARISON_CANVAS_ID}`) as HTMLCanvasElement;
  }
