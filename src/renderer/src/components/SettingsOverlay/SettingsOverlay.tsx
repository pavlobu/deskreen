...
export default function SettingsOverlay(props: SettingsOverlayProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    return () => {
      window.electron.ipcRenderer.removeListener('settings-overlay-close', handleClose);
    };
  }, [handleClose]);

  useEffect(() => {
    const getLatestVersion = async (): Promise<void> => {
      const gotLatestVersion = await window.electron.ipcRenderer.invoke('get-latest-version');
      if (gotLatestVersion !== '') {
    ...
    const getCurrentVersion = async (): Promise<void> => {
      const gotCurrentVersion = await window.electron.ipcRenderer.invoke('get-current-version');
      ...
      <div style={{ width: '100%' }}>
        {hasUpdate ? (
          <div className={classes.updateCalloutWrapper}>
            ...
            >
              <Text style={{ fontWeight: 600 }}>{t('deskreen-ce-update-is-available')}</Text>
        ...
        <Row center="xs" middle="xs" style={{ marginTop: '40px', width: '100%' }}>
          <div>
            <Col xs={12}>
              <img
      ...
      <div className={classes.overlayInnerRoot}>
        ...
        >
          <CloseOverlayButton
            ...
            isDefaultStyles
          />
          <Tabs animate id="TabsExample" key="vertical" renderActiveTabPanelOnly vertical>
            <Tab
              id="rx"
              title=""
