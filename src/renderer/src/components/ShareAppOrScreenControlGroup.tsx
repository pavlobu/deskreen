...
export default function ShareAppOrScreenControlGroup(props: ShareAppOrScreenControlGroupProps) {
  const {
  ...
  const [isChooseAppOrScreenOverlayOpen, setChooseAppOrScreenOverlayOpen] = useState(false);

  const [isEntireScreenToShareChosen, setEntireScreenToShareChosen] = useState(false);
  ...
  const handleWaylandShare = useCallback(
    async (mode: 'screen' | 'window') => {
      try {
        const sourceId: string | null = await window.electron.ipcRenderer.invoke(
        ...
        await window.electron.ipcRenderer.invoke(IpcEvents.SetDesktopCapturerSourceId, sourceId);
        if (mode === 'screen') {
          handleNextEntireScreen();
        } else {
          handleNextApplicationWindow();
        ...
        console.error('failed to acquire desktop capture source via portal', error);
      }
    },
  ...
  }, [handleOpenChooseAppOrScreenOverlay, handleWaylandShare, isWaylandSession]);
  ...
  return (
    <>
      ...
      >
        ...
        <Button active className={classes.orDecorationButton} style={{ zIndex: 999 }}>
          {t('or')}
        </Button>
      </ControlGroup>
